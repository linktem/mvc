...
class LibMemcached {
    ...
    private function auto_connect() {
        ...
        $this->add_server($ips, 'xiejun');
    }

    /**
     * @Name: add_server
     * @param:none
     * @todu 连接memcached server
     * */
    public function add_server($server, $prefix) {
        $this->m = new Memcached($prefix);
        $this->m->setOption(Memcached::OPT_LIBKETAMA_COMPATIBLE, TRUE);
        if (!count($this->m->getServerList())) {
            $this->m->addServers($server);
        }
    }
    ...
    public function get($key = NULL) {
        if ($this->m) {
            ...
            if (is_array($key)) {
                ...
                $result = $this->m->getMulti($key);
            } else {
                $result = $this->m->get($this->key_name($key));
            }
...
//END append
    ...
    public function cas($key = NULL, $value = NULL, $expiration) {
        do {
            $key = $this->key_name($key);
            if (is_null($expiration)) {
                $expiration = MEMCACHE_EXPIRATION;
            }
            /* 获取列表以及它的标记 */
            $this->m->get($key, null, $cas);
            /* 如果列表不存在， 创建并进行一个原子添加（如果其他客户端已经添加， 这里就返回false） */
            if ($this->m->getResultCode() == Memcached::RES_NOTFOUND) {
                $this->m->add($key, $value, $expiration);
                /* 以cas方式去存储， 这样当其他客户端修改过， 则返回false */
            } else {
                $this->m->cas($cas, $key, $value, $expiration);
            }
        } while ($this->m->getResultCode() != Memcached::RES_SUCCESS);
    }
