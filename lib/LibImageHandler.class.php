 ...
 * 生成缩略图片使用方法说明
 * 
  $imgHandler_Obj = new LibImageHandler();
  //设置图片路径
  $img_path = 被缩略的图片物理路径;
  $imgHandler_Obj->setSrcImg($img_path);
  //设置缩略图边框
  $imgHandler_Obj->setDstImgBorder(0, '#000');
  $imgHandler_Obj->setDstImg($img_path);
  //缩略图比例
  $imgHandler_Obj->createImg(图片宽度, 图片高度);
  unset($imgHandler_Obj);
 * 
 */
class LibImageHandler {

    protected $dst_img; // 目标文件
    protected $h_src; // 图片资源句柄
    protected $h_dst; // 新图句柄
    protected $h_mask; // 水印句柄
    protected $img_create_quality = 100; // 图片生成质量
    protected $img_display_quality = 100; // 图片显示质量,默认为75
    protected $img_scale = 0; // 图片缩放比例
    protected $src_w = 0; // 原图宽度
    protected $src_h = 0; // 原图高度
    protected $dst_w = 0; // 新图总宽度
    protected $dst_h = 0; // 新图总高度
    protected $fill_w; // 填充图形宽
    protected $fill_h; // 填充图形高
    protected $copy_w; // 拷贝图形宽
    protected $copy_h; // 拷贝图形高
    protected $src_x = 0; // 原图绘制起始横坐标
    protected $src_y = 0; // 原图绘制起始纵坐标
    protected $start_x; // 新图绘制起始横坐标
    protected $start_y; // 新图绘制起始纵坐标
    protected $mask_word; // 水印文字
    protected $mask_img; // 水印图片
    protected $mask_pos_x = 0; // 水印横坐标
    protected $mask_pos_y = 0; // 水印纵坐标
    protected $mask_offset_x = 5; // 水印横向偏移
    protected $mask_offset_y = 5; // 水印纵向偏移
    protected $font_w; // 水印字体宽
    protected $font_h; // 水印字体高
    protected $mask_w; // 水印宽
    protected $mask_h; // 水印高
    protected $mask_font_color = '#000000'; // 水印文字颜色
    protected $mask_font = 5; // 水印字体
    protected $font_size; // 尺寸
    protected $mask_position = 0; // 水印位置
    protected $mask_img_pct = 50; // 图片合并程度,值越大，合并程序越低
    protected $mask_txt_pct = 50; // 文字合并程度,值越小，合并程序越低
    protected $img_border_size = 0; // 图片边框尺寸
    protected $img_border_color; // 图片边框颜色
    protected $_flip_x = 0; // 水平翻转次数
    protected $_flip_y = 0; // 垂直翻转次数
    protected $cut_type = 0; // 剪切或缩小类型
    protected $img_type; // 文件类型
    // 文件类型定义,并指出了输出图片的函数
     ...
     * 取得图片的宽
     */
    private function getImgWidth($src) {
        return imagesx($src);
    }
     ...
     * 取得图片的高
     */
    private function getImgHeight($src) {
        return imagesy($src);
    }
        ...
        if (2 == $num) {
            ...
            switch ($this->cut_type) {
                case 0:
                    ...
                    // 目标尺寸
                    $this->dst_w = $this->fill_w + $this->img_border_size * 2;
                    $this->dst_h = $this->fill_h + $this->img_border_size * 2; //(原代码: 补偿round误差 消除白边; $this->fill_h + $this->img_border_size*2 - 10)
                    break;
